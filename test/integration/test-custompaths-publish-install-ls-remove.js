 ...
 * custom package install path and base URL
...
exports['project with package.json'] = {
    ...
    setUp: function (callback) {
        ...
        ncp('./fixtures/project-custompaths', this.project_dir, callback);
    },
    ...
    'publish, install, ls, remove': function (test) {
        ...
        async.series([
            ...
            function (cb) {
                ...
                var b = fs.readFileSync(path.resolve(
                    that.project_dir,
                    'public/js/vendor/package-one/main.js'
                ));
                test.equal(a.toString(), b.toString());
                var c = fs.readFileSync(path.resolve(pkgtwo, 'two.js'));
                var d = fs.readFileSync(path.resolve(
                    that.project_dir,
                    'public/js/vendor/package-two/two.js'
                ));
                test.equal(c.toString(), d.toString());

                // make sure the requirejs config includes the new package
                var cfg = utils.freshRequire(path.resolve(
                    that.project_dir, 'public/js/vendor/require.config'
                ));
                ...
                test.same(packages, [
                    {
                        name: 'package-one',
                        location: 'js/vendor/package-one'
                    },
                    {
                        name: 'package-two',
                        location: 'js/vendor/package-two',
                        main: 'two.js'
                    }
                ]);
                cb();
