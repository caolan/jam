    ...
    return function () {
        for (var i = 0; i < levels.length; i++) {
            if (levels[i] === exports.level) {
                return fn.apply(exports, arguments);
            }
        }
    };
};
...
/**
 * Logs verbose messages, using util.inspect to show the properties of objects
 * (logged for 'verbose' level only)
 */

exports.verbose = forLevels(['verbose'], function (label) {
    var args, withArgs;

    args = Array.prototype.slice.call(arguments, 0).map(function(arg) {
    ...
    if (label && withArgs) {
        console.log(yellow(label + ' ') + args.slice(1).join("\n"));
    } else {
 ...
 * (logged for 'debug', 'verbose' levels)
 */

exports.debug = forLevels(['debug', 'verbose'], function (label, val) {
    if (val === undefined) {
        val = label;
        label = null;
    }
...
exports.info = forLevels(['info', 'debug', 'verbose'], function (label, val) {
    if (val === undefined) {
        val = label;
        label = null;
    }
...
exports.warning = forLevels(['warning', 'info', 'debug', 'verbose'], function (msg) {
