...
exports.publish = function (path, repository, /*opt*/options, callback) {
    ...
    settings.load(path, function (err, cfg) {
        ...
        async.series({
            ...
            session: function (cb) {
                var parsed = urlParse(repository);
...
exports.search = function (settings, repo, q, /*optional*/limit, callback) {
    if (!callback) {
        callback = limit;
        limit = 10;
    }
    r = repo.replace(/\/$/,'') + '/';
    var s_url = getSearchUrl( r, settings );
    ...
    request.get({url: s_url, json: true, qs: qs}, function (err, res, data) {
...
function getSearchUrl( r, settings ) {
    var search_prefix = '/_fti/local/jam_packages/_design/jam-packages/packages',
        search_url;

    if (
        settings
     && settings.search_urls
     && settings.search_urls[ r ]
     && settings.search_urls[ r ] !== ''
     && typeof( settings.search_urls[ r ] ) === 'string'
    ) {
        search_url = settings.search_urls[ r ];
    } else {
        search_url = url.resolve( r, search_prefix );
    };
