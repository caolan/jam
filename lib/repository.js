...
exports.createDoc = function (type, user, time, cfg, tfile, readme, callback) {
    var doc = {
        _id: cfg.name,
        name: cfg.name,
        type: type,
...
exports.publish = function (type, path, repository, /*opt*/options, callback) {
    if (!callback) {
        callback = options;
        options = {};
    }
    ...
    settings.load(path, function (err, cfg) {
        ...
        async.series({
            ...
            cache: async.apply(cache.update, type, cfg.name, cfg.version, path),
        ...
        function (err, results) {
            ...
            if (!curr) {
                return exports.createDoc(
                    type, user, time, cfg, tfile, readme, function (err, doc) {
...
exports.unpublish = function (repository, name, version, options, callback) {
    ...
    function removeVersionFromCache(type, v) {
        return function (err) {
            if (err) {
                return callback(err);
            }
            cache.clear(type, name, v, callback);
        };
    }
    ...
    function removeVersion(v, data) {
        ...
        if (Object.keys(data.versions).length) {
            db.save(name, data, removeVersionFromCache(data.type, v));
        }
        else {
            // no more version remaining
            db.delete(name, data._rev, removeVersionFromCache(data.type));
        }
    }
    exports.get(repository, name, function (err, data) {
        ...
        if (!version) {
            return db.delete(
                name, data._rev, removeVersionFromCache(data.type)
            );
        }
...
exports.fetch = function (type, name, version, repos, callback) {
    logger.debug('fetching ' + type, name + ' (' + version + ')');

    cache.get(type, name, version, function (err, tarfile, cachedir) {
        if (cachedir && tarfile) {
        ...
        exports.resolve(name, version, repos, function (err, v, cfg, repo) {
            ...
            cache.get(type, name, v, function (err, c_tfile, c_cdir) {
                if (err) {
                    return callback(err);
                }
                if (c_tfile && c_cdir) {
                ...
                exports.download(url, function (err, tarfile) {
                    ...
                    cache.moveTar(type, name, v, tarfile,
...
exports.download = function (file, callback) {
    ...
    mkdirp(exports.TMP_DIR, function (err) {
        if (err) {
            return callback(err);
        }
        var headers = {};
