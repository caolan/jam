        ...
        async.parallel(
            ...
            function(err, result) {
                ...
                if (!isUnique) {
                    ...
                    //if (uniq.length > 1) {
                    logger.info('git', 'multiple refs was found for the commitish "' + spec.value + '": ' + _.pluck(list, 'type').join(', ') + '; during order using: ' + commitish.type);
                    //}
                }
...
exports.getSettings = function(uri, callback) {
    var spec, path;
...
exports.availableVersions = function (name, uri, callback) {
    ...
    exports.get(path, function(err, repository) {
        ...
        repository.resolve(spec, function(err, commitish) {
            ...
            if (!commitish) {
                logger.warning('package "' + name + '" from ' + path + ' does not provide any ref, tag or commit like "' + spec.value + '"');
                callback(null, {});
                return;
            }
