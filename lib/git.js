...
Mutex.prototype.unlock = function(cb) {
    if (this.locked == false) {
        return setTimeout(function() {
            cb(new Error("unlock of unlocked mutex"))
        }, 0);
    }
...
Repository.prototype = {
    ...
    fetch: (function() {
        ...
        command = _.template('git fetch && git fetch --tags');
            ...
            [
                function(next) {
                    ...
                    path = [
                        exports.CACHE_DIR,
                        uriParsed.hostname,
                        _.isString(uriParsed.pathname) ? uriParsed.pathname.substr(1).replace(/:/gi, "").replace(/\.git$/gi, "") : hash(uri)
                    ].join("/");
                    ...
                    fs.stat(path, function(err, stats) {
                        ...
                        if (!isEmpty && stats.isDirectory()) {
                            logger.debug("git", "using cached clone of " + uriClone + " from " + path);
                            next(null, (exports.repositories[uriClone] = new Repository(path)), true);
                            return;
                        }

                        logger.debug("git", "cloning " + uriClone);
                        run(command({ uri: uriClone, to: path }), function(err) {
                            if (err) {
                                callback(err);
                                return;
                            }
...
exports.define = function (uri, callback) {
    ...
    async.waterfall(
        [
            // retrieve repo from path
            async.apply(exports.get, path),

            // get pkg from repo at given spec
            function(repository, next) {
                async.waterfall(
                    [
                        // take repo's lock to prevent data races
                        function(next) {
                            logger.verbose("git", "locking " + path);
                            repository.mu.lock(function(err) {
                                if (err) {
                                    return next(err);
                                }

                                logger.verbose("git", "locked " + path);
                                next();
                            });
                        },

                        // resolve spec to commitish
                        async.apply(repository.resolve.bind(repository), spec),

                        // switch to commitish
                        function(commitish, next) {
                            if (!commitish) {
                    ...
                    function(err, def) {
                        // release lock
                        logger.verbose("git", "unlocking " + path);
                        repository.mu.unlock(function(e) {
                            if (e) {
                                log.verbose("git", "unlock " + path + " error: " + e);
                            } else {
                                logger.verbose("git", "unlocked " + path);
                            }
...
exports.snapshot = function(def, callback) {
    logger.verbose("git", "creating snapshot for " + def.path);

    exports.get(def.path, function(err, repository) {
        if (err) {
            return callback(err);
        }

        logger.verbose("git", "locking " + def.path);
        repository.mu.lock(function(err) {
            if (err) {
                return callback(err);
            }

            logger.verbose("git", "locked " + def.path);

            repository.snapshot(def.commitish, function(err, path) {

                repository.mu.unlock(function(e) {
                    if (e) {
                        log.verbose("git", "unlock " + def.path + " error: " + e);
                    } else {
                        logger.verbose("git", "unlocked " + def.path);
                    }

                    callback(err || e || null, path);
                });
