...
exports.writeJSON = function (path, obj, callback) {
    var str;
...
exports.download = function (file, target, callback) {
    ...
    mkdirp(path.dirname(target), function (err) {
        ...
        var req = request({
            uri: url.format(_.extend(urlinfo, { pathname: urlinfo.pathname.split("/").map(encodeURIComponent).join("/") })),
            method: 'GET',
            headers: headers,
            proxy: proxy
        }, function() {
        ...
        req.on('response', function (response) {
            if (response.statusCode >= 300) {
                this.abort();
                return callback(exports.statusCodeError(response.statusCode));
            }
