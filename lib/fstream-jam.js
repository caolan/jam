...
function Packer (props) {
  ...
  if (props.packageInfo && props.packageInfo.jam) {
      this.jamInclude = props.packageInfo.jam.include;
  }
...
Packer.prototype.applyIgnores = function (entry, partial, entryObj) {
  ...
  if (this.jamInclude) {
      for (var i = 0; i < this.jamInclude.length; i++) {
        if (minimatch(entry, this.jamInclude[i])) {
            return true;
        }
        // test for subpaths, eg jam.include = ['foo'], entry = 'foo/bar.js'
        if (utils.isSubPath(this.jamInclude[i], entry)) {
            return true;
        }
      }
      return !!(partial);
