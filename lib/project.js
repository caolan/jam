...
/**
 * Build up the string to be included in the require.config.js
 *
 * @param {Object} requireOpts - options availabe for requirejs.config
 * @param {Object} shim - these are handed in and merged
 */

exports.includeRequireJsConfig = function(requireOpts, dataInclude, indent){
    // sssshhh, nobody will notice this gashtly code
    var val = _.extend(requireOpts || {}, { 'package': 'XX', 'shim': 'YY'});
    var optsSrc = JSON.stringify(val, null, indent || 4);
    optsSrc = optsSrc.replace('"XX"', 'jam.packages');
    optsSrc = optsSrc.replace('"YY"', 'jam.shim');
    return optsSrc;
};

/**
 * Merge the shims to included in the require.config.js
 *
 * @param {Object} requireOptsShim - options availabe for requirejs.config
 * @param {Object} jamShim - these are handed in and merged
 */
 
exports.mergeShims = function(requireOptsShim, jamShim){
  // Note: this is likely to need to extended for shared properties beyond shim
    return _.extend({}, requireOptsShim, jamShim);
};
...
exports.updateRequireConfig = function (package_dir, baseurl, requireConfig, callback) {
    var packages = [];
    var shims = {};

    var requireConfig = typeof requireConfig === 'function' ? {} : requireConfig || {};
    ...
    exports.getAllPackages(package_dir, function (err, pkgs) {
        ...
        utils.getJamVersion(function (err, version) {
            ...
            data.shims = exports.mergeShims(requireConfig.shim, data.shim);
            var cfg = exports.includeRequireJsConfig(requireConfig);
