...
'  jam install ./path/to/jam.json (reinstall listed packages)\n' +
'\n' +
'Parameters:\n' +
...
'  --package-dir  Output directory (defaults to "./jam")';
    ...
    fs.stat(pkg, function (err, stats) {
        if (err) {
            ...
            if (/.*\.tar\.gz$/.test(pkg) || /.*\.tgz$/.test(pkg)) {
                ...
                return exports.installList(pkg, opt, callback);
            }
        }
        else {
...
exports.installList = function (filename, opt, callback) {
    if (opt.no_deps) {
        return callback();
    }
...
exports.installRepo = function (name, range, opt, callback) {
    repository.fetch(name, range, opt.package_repositories,
...
exports.installFile = function (filename, opt, callback) {
    exports.prepareFile(filename, function (err, cfg, tdir, tfile) {
        ...
        exports.initDir(opt, function (err, opt, rootcfg) {
            ...
            rootcfg.dependencies[cfg.name] = null;
            project.writeMeta(opt.target_dir, rootcfg, function (err, rootcfg) {
                if (err) {
                    return callback(err);
                }
                var sources = [
