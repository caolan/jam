...
exports.usage = '' +
    ...
    '      --save         Save dependencies to the package.json\n' +
...
exports.installPackages = function (cfg, names, opt, callback) {
    ...
    async.reduce(
        ...
        function (err, packages) {
            ...
            var save = _.reduce(packages, function(save, p, name) {
                for (var i = 0, len = p.requirements.length; i < len; i++) {
                    var req = p.requirements[i];
                    var ok = true;

                    // if it is actual requirement (aka not shadowed)
                    ok = ok && req.enabled;
                                    
                    // if it is just numeric version
                    ok = ok && req.range.source == req.range.range;

                    // if requirement is from cli
                    ok = ok && req.path.parents.length == 1;
                    ok = ok && req.path.parents[0] == "_cli";

                    if (ok) {
                        save[name] = p.current_version;
                        return save;
                    }
                }
            ...
            var tasks = [
                async.apply(project.updateRequireConfig, opt.target_dir, opt.baseurl)
            ];

            if (opt.save && Object.keys(save).length > 0) {
                tasks.push(async.apply(project.updatePackageJSON, opt.proj_dir, false, save));
            }
...
exports.install = function (cfg, pkg, opt, callback) {
    ...
    fs.stat(pkg, function (err, stats) {
        ...
        if (stats.isFile() && (/.*\.tar\.gz$/.test(pkg) || /.*\.tgz$/.test(pkg))) {    
...
exports.extractValidVersion = function(pkg, name, cb) {
    ...
    versions = _.chain(pkg.versions)
        ...
        .first()
        .result('versions')
        .uniq(function(version, index) {
            switch (version.source) {
                // filter git duplicates
                case 'git': {
                    return version.git.uri;
                }

                // in other case leave as is
                default: {
                    return index;
                }
            }
    ...
    if (versions.length > 1) {
        // if we have only multiple repositories
        // pick the lowest priority from jamrc
        versionsSource = _.chain(versions).pluck("source").uniq().value();
        if (versionsSource.length == 1 && versionsSource[0] == 'repository') {
            return jamrc.load(function(err, rc) {
                var version;
                ...
                version = _
                    ...
                    .sortBy(function(version) {
                        return _.indexOf(rc.repositories, version.repository)
                    })
                    .first()
                    .value();
                ...
                async.forEachLimit(
                    ...
                    function (def, cb) {
                        ...
                        var next = function(err) {
                            ...
                            def.pkg.installed_version = version;
                            cb();
                        };
...
exports.installGit = function(name, def, opt, callback) {
    git.snapshot(def, function(err, path) {
        if (err) {
            callback(err);
            return;
        }
