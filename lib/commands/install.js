...
'  -s, --strict       Strict mode for checking non stable dependencies in subpackages\n' +
'  -p, --production   Restricts to install from git repositories\n' +
...
exports.gitSource = function(meta) {
    return function(def, callback) {
        ...
        // skip non git paths
        if (!(/^git(\+[a-z]+)?:/.test(uri = range))) {
            callback(null, {});
            return;
        }

        normalized = uri.replace(/^git(\+[a-z]+:)?(\/\/.+)/, function(str, protocol, uri) {
            return ( protocol ? protocol.substr(1) : "git:" ) + uri;
        });

        logger.info('git', 'checking "' + name + '@' + normalized + '"');
...
exports.stableVersionsPackageValidator = function(pkg, name, cb) {
    var rootRange, rootRangeParsed, rootRangeIsStrict, notStrictRanges;

    if (!(rootRange = pkg.ranges._root)) {
        rootRangeIsStrict = false;
    } else {
        rootRangeIsStrict = (rootRangeParsed = semver.validRange(rootRange)) == rootRange;
    }

    if (!rootRangeIsStrict) {
        notStrictRanges = _.chain(pkg.ranges)
            .omit('_root')
            .map(function(range, parent) {
                var parsed;

                if ((parsed = semver.validRange(range)) == range) {
                    return null;
                }
...
exports.extractValidVersion = function(pkg, name, cb) {
    ...
    versions = _.chain(pkg.versions)
        .where({ version: curr })
        .groupBy('source')
        .reduce(function(list, versions, source) {
            list.push({ source: source, versions: versions });
            return list;
        }, [])
        .sortBy(function(obj) {
            var index;
            return (index = _.indexOf(priority, obj.source)) == -1 ? priority.length : index;
        })
        .first()
        .result('versions')
        .value();
                            ...
                            []
                                ...
                                .concat(versionValidators.map(function(validator) {
                                    return function(version, callback) {
                                        validator.call(null, version, pkg, name, callback);
                                    };
                                })),
