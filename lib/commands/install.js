...
exports.gitSource = function(meta) {
    return function(name, callback) {
        ...
        // skip non git paths
        if (!(/^git(\+[a-z]+)?:/.test(uri = project.getJamDependencies(meta)[name]))) {
            callback(null, {});
            return;
        }
        ...
        normalized = uri.replace(/^git(\+[a-z]+:)?(\/\/)(.+)/, function(str, protocol, slashes, uri) {
            var schema;

            schema = (protocol ? protocol.substr(1) : "git:");
...
exports.installTree = function (packages, opt, callback) {
    ...
    async.forEachLimit(names, 5, function (name, cb) {
        ...
        switch (packages[name].versions[curr].source) {
            case 'repository': {
            ...
            case 'git': {
                exports.installGit(name, packages[name].versions[curr].def, opt, cb);
                break;
            }

            case 'github': {
                cb('GitHub source is not implemented yet');
                break;
            }

            case 'url': {
                cb('URL source is not implemented yet');
                break;
            }

            case 'file': {
                cb('File source is not implemented yet');
                break;
            }
