...
exports.run = function (settings, args) {
    ...
    var cwd = process.cwd();
    exports.initDir(settings, cwd, opt, function (err, opt, cfg) {
        if (err) {
        ...
        if (!opt.target_dir) {
            if (cfg.jam && cfg.jam.packageDir) {
                opt.target_dir = cfg.jam.packageDir;
            }
            else {
                opt.target_dir = settings.package_dir;
            }
        }
        if (!opt.baseurl) {
            if (cfg.jam && cfg.jam.baseUrl) {
                opt.baseurl = cfg.jam.baseUrl;
            }
            else {
                opt.baseurl = settings.baseUrl;
            }
        }
...
exports.reinstallPackages = function (cfg, opt, callback) {
    ...
    tree.build(pkg, sources, function (err, packages) {
        ...
        exports.installTree(packages, opt, function (err) {
            ...
            project.updateRequireConfig(opt.target_dir, opt.baseurl, packages,
               function (err) {
                    if (err) {
                        return logger.error(err);
                    }
