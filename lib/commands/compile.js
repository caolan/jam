...
exports.usage = '\n' +
    ...
    '                      almond option.\n' +
    '  -a, --almond        Use the lightweight almond shim instead of RequireJS,\n' +
...
exports.filterPackages = function (cfgpackages, pkgdir, names, callback) {
    async.filter(names, function (n, cb) {
        var pkg = _.detect(cfgpackages, function (pkg) {
            return pkg.name === n;
        });
        if (pkg && pkg.main) {
            // main property defined
            return cb(true);
        }
        else {
            // check main.js exists, otherwise optimizer fails
            fs.exists(path.join(pkgdir, n, 'main.js'), cb);
        }
    }, callback);
};
...
exports.compile = function (opt, callback) {
    ...
    install.initDir(opt.settings, opt.cwd, opt, function (err, opt, cfg, pdir) {
        ...
        exports.filterPackages(
            packages, opt.pkgdir, pkgnames, function (valid_names) {
                if (!opt.includes.length) {
                    // compile all installed modules by default
                    opt.includes = valid_names;
                }
                if (!opt.output) {
                ...
                try {
                    requirejs.optimize(config, function (build_response) {
                        if (/^Error: Error:/.test(build_response)) {
                            // TODO: try to get better error handling added
                            // upstream
                            return callback(build_response);
                        }
