...
exports.extend = function (parents, pkg, sources, translators, packages, callback) {
    var name, version;
...
exports.addDependency = function (parents, parent, name, range, translation, sources, translators, packages, cb) {
    var dep, def, sourceIndex, sourcesLength, partialMatch, r, path;
    ...
    dep.ranges[path] = (r = new Range(translation, range));

    logger.verbose("tree", 'new range: "' + path + '" requires "' + name + '"@' + r.toString());
    ...
    async.whilst(
        ...
        function(err) {
            ...
            if (match) {
                ...
                dep.ranges = _.reduce(dep.ranges, function(memo, range, path) {
                    if (range === match) {
                        memo[path] = range;
                    } else {
                        clear.push(path);
                ...
                logger.verbose("tree", 'removed requirers from ranges of "' + name + '": ' + clear.join(", "));

                clear.forEach(function(clear) {
                    _.forEach(packages, function(dep, name) {
                        try {
                            delete dep.ranges[clear];
                            logger.verbose("tree", 'removed requirer from ranges of "' + name + '": ' + clear);
                        } catch (err) {
                            //
                        }
                    });
...
exports.dependencyError = function (name, dep) {
    ...
    for (parent in dep.ranges) if (dep.ranges.hasOwnProperty(parent)) {
        ...
        ranges += '\t' + parent + ' requires ' + range.toString() + '\n';
    }
