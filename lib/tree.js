...
exports.extend = function (parents, pkg, sources, translators, packages, callback, isInner) {
    ...
    if (cfg.browser && cfg.browser.dependencies) {
        _.extend(depobj, cfg.browser.dependencies);
    }
    if (cfg.jam && cfg.jam.dependencies) {
        _.extend(depobj, cfg.jam.dependencies);
    }
 ...
 *
 * @param {Array} parents - the name of the packages which depends on this one (path)
...
exports.addDependency = function (parents, name, r, sources, translators, packages, cb) {
    exports.translateRange(name, r, translators, function(err, translation) {
        ...
        var i, len, req, dup;
        for (i = 0, len = dep.requirements.length; i < len; i++) {
            req = dep.requirements[i];
            dup = true;
            dup = dup && req.path.toString() == path.toString();
            dup = dup && req.range.range == range.range;
            dup = dup && req.range.source == range.source;

            if (dup) {
                logger.verbose("tree", 'requirement already exists: ' + req.toString());
...
exports.translateRange = function(name, range, translators, cb) {
